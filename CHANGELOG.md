...
- `ai.Request.Hosted` is answered with `ai.ErrNoHosted` before the request
  leaves, because the provider's own searches are not reachable from the chat endpoint this
  package speaks, and its documentation leaves more than one plausible request
  shape.
...
- Native `ChatCompletion` and `ChatCompletionStream` exposing the full chat
  option set (response_format, seed, n, ...).
- Image generation (`GenerateImage`) and models (`Models`, `GetModel`).
